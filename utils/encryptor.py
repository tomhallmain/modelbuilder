...
def _determine_encryptor(service_name, app_identifier, override_stored_type=False):
    ...
    if not override_stored_type and stored_type == "quantum":
        if KeyEncapsulation:
            ...
            raise RuntimeError(
                "Stored keys use the quantum encryptor but OQS (liboqs-python) is not available. "
                "Install optional post-quantum deps (see requirements-optional.txt) or reset keys."
            )
